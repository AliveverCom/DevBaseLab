...
namespace Alivever.Com.DevBasic.BasicLib.DbCtrl
{
	...
	/// DbConnMgr 的摘要说明。
	/// </summary>
	public class COdbcMgr : CDbMgrBase
	{
...
//		}//string DBconnStr
		...
		//private OdbcConnection m_DbConn = null ;

		//"Driver={MySQL ODBC 3.51Driver};Server=127.0.0.1;Option=16834;Database=mydb;Uid=root;Pwd=;"
		//private string m_DBconnStr = "";
		...
		public COdbcMgr( string _DBconnStr )
		{
			this.DBconnStr = _DBconnStr;
		}

		public COdbcMgr( string _DbDriver, string _DbServer, string _DbDatabase, string _DbUid, string _DbPwd)
		{
		...
		public override bool OpenDB( string _DBconnStr )
		{
			...
			m_DbConn = new OdbcConnection(_DBconnStr);
			return OpenDB( true );
		}//ConnectDB(1)
		...
		public override bool OpenDB(  bool _bForceOpen )
		{
			...
			if ( m_DbConn == null)
			{
				if ( !_bForceOpen )
				{
					m_LastError = "COdbcMgr hasn't been connected.";
					GSdkMLog.At( GT.pkgName).Write("COdbcMgr.OpenDB(1)",1 , "COdbcMgr hasn't been connected.\n");

					return false;
				}
				else
					m_DbConn = new OdbcConnection( this.DBconnStr );
			}
		...
		*/
		...
		public OdbcDataReader RunSqlMultiResult(  string _sqlStr )
		{
			if ( this.m_DbState != EDbState.Open && !this.OpenDB( true ) )
			{
				GSdkMLog.At( GT.pkgName).Write("COdbcMgr.RunSqlMultiResult(1)",1 , "Can't run SQL=\""+ _sqlStr +"\",because:" + m_LastError + "\n");
				m_LastError = "COdbcMgr can be connected or opened.";
				return null;
			}

			OdbcDataReader odReader;
			try
			{
				OdbcCommand myOdbcCommand = new OdbcCommand( _sqlStr , (OdbcConnection)m_DbConn);
				odReader = myOdbcCommand.ExecuteReader();
			}
			catch( Exception ex)
			{
				m_LastError = ex.Message ;
				GSdkMLog.At( GT.pkgName).Write("COdbcMgr.RunSqlMultiResult(1)",1 , "run SQL error. SQL=\""+ _sqlStr +"\",Message=" + m_LastError + "\n");
				return null;
			}
		...
		public override bool RunSqlSingleResult(  string _sqlStr , ref Object _RstObj )
		{
			if ( this.m_DbState != EDbState.Open && !this.OpenDB( true ) )
			{
				GSdkMLog.At( GT.pkgName).Write("COdbcMgr.RunSqlSingleResult(1)",1 , "Can't run SQL=\""+ _sqlStr +"\",because:" + m_LastError + "\n");
				m_LastError = "COdbcMgr can be connected or opened.";
				return false;
			}
			...
			{
				OdbcCommand myOdbcCommand = new OdbcCommand( _sqlStr , (OdbcConnection)m_DbConn);
				_RstObj = myOdbcCommand.ExecuteScalar();
			}
			catch( Exception ex)
			{
				m_LastError = ex.Message ;
				GSdkMLog.At( GT.pkgName).Write("COdbcMgr.RunSqlSingleResult(1)",1 , "run SQL error. SQL=\""+ _sqlStr +"\",Message=" + m_LastError + "\n");
				return false;
			}
		...
		public override bool RunSqlNoReturn( string _sqlStr )
		{
			if ( this.m_DbState != EDbState.Open && !this.OpenDB( true ) )
			{
				GSdkMLog.At( GT.pkgName).Write("COdbcMgr.RunSqlNoReturn(1)",1 , "Can't run SQL=\""+ _sqlStr +"\",because:" + m_LastError + "\n");
				m_LastError = "COdbcMgr can be connected or opened.";
				return false;
			}
			...
			{
				m_LastError = ex.Message ;
				GSdkMLog.At( GT.pkgName).Write("COdbcMgr.RunSqlNoReturn(1)",1 , "run SQL error. SQL=\""+ _sqlStr +"\",Message=" + m_LastError + "\n");
				return false;
			}
